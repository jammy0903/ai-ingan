...
- ⚠️ **잔여 검증(실기기/iOS)**: 위는 에뮬레이터 결과. **실제 안드로이드 폰 + (이후) iOS Safari webview에서 OAuth 재확인 필요** — 구글의 webview 정책/기기차로 막힐 가능성 대비 **딥링크 해법은 폐기 말고 '폴백'으로 보존**(아래 OAuth 항목 유지).
...
- [x] **🔑 OAuth in-webview(최대 기술 리스크) — 에뮬레이터 검증 완료(2026-06-25)**: server.url + 기본 webview에서 구글 로그인·세션 생성 정상. 딥링크 불필요. (게이트 레이스만 v1.1.16에서 수정.)
  - [ ] **실기기/iOS 재확인** — 에뮬 통과 ≠ 모든 기기 보장. 실폰 + iOS에서 1회 더 확인.
...
2. ✅ **Capacitor 셸 PoC + webview OAuth 검증**(§2) — **완료(2026-06-25)**. 셸 빌드·실행·OAuth·세션 다 정상, 게이트 레이스 v1.1.16 수정. 최대 리스크 (b) 해소. (실폰/iOS 재확인만 남음.)
3. **(다음) 실폰에서 v1.1.16 재검증** → **AdMob 리워드 연결**(§3) — 핵심 수익화 동작 확인.
...
> 남은 큰 리스크: **(a) 비공개 테스트 20명/14일**, **(c) 서명 연속성**(기존 패키지 업데이트 거부 방지, §2). ~~(b) webview OAuth~~ → ✅ PoC로 해소(에뮬). 실폰/iOS 재확인만.
