...
- MVP core loop works **+ 2026-06-18 dopamine/economy overhaul + 2026-06-19 onboarding redesign & Phase 2 content**: walk + tap (steps/sec-proportional tap, combo ×7, crit), cells-style **sequential** graph (category gate · body last), meeting/**endless reunion sink** (×2 achievement bonus every 5 levels), **milestone multipliers → exponential income shown in K/M/B**, **onboarding 'tap-to-wake'** (lying→standing rotation · empty core · naming after Sam quest), save + Google login, ending cinematic, PWA, **emotion27 content 5-line + pick pickup gag**. **Live build v1.0.13 at aingan.click (Cloudflare auto-deploy on push).** Core design locked (`CLAUDE.md` §6). Gap analysis + roadmap = `gap-analysis.md`.
- **Lightweight strangler refactor ①~④ DONE** (save versioning, `data.js`, `balance.js`, SW bump rule). `data.js`/`balance.js` = global `<script src>` (NOT ES modules), loaded before inline logic; engine/view stay in index.html.
- **Rate is computed from levels** now: `baseRate()` = 1 + Σ `nodeOut(n,lv)` (gen×lv×`achieveMult`); `effRate()` = baseRate × `rateMult()` (milestones). Legacy `S.rate` field kept for save-compat only. Tap = `effTap()`.
- **Versioning = semver `1.0.N`**: bump 3 places together — `sw.js` `CACHE="aingan-1.0.N"` · `manifest.json` `"version"` · `index.html` `#appVer`.
- **Decided but NOT built — target sentence (A-flip × C 4-clause):** during play reads as a plea「…그래도 걷는 내가 — 사람이 되게 해줘?」(blank cycles comedic wrong guesses), Sam-reunion final piece flips it to「…벌써 사람이잖아.」. 4 clauses light up as each of the 4 categories completes. Plus 거지키우기-style "잼" (emotion **pickup** + **misread gags** + **achievement slot**). Content draft = `content-emotions-draft.md` (emotions only; body excluded so far).
- **Next:** ① ✅ DONE — wired `content-emotions-draft.md` into `data.js` (`pick` field + `lines` 3→5) + pick pickup gag (Phase 2 complete, v1.0.13). ② (next) target-sentence engine hooks. ③ Phase 4 prestige shop (spend `depth`). ④ economy tuning. (See `gap-analysis.md`.)
